...
Realizar un programa C++ que lea un numero real (correspondiente a un monto) y convertir
...
string ConvertirPalabras(int numero, bool terminacion, int divisor, string resultado); //Convertir un grupo de 3 o menos digitos a palabras
...
string EscribirNumero(int numero, int contador, string resultado); //retorna el numero completo en palabras
...
int main(){
	...
	do{
		cout<<"-----------------------------------------------------\n";
		cout<<"          CONVERTIR UN NUMERO A PALABRAS\n";
		cout<<"-----------------------------------------------------\n";
		
		CapturarEntrada("Ingrese el numero para convertir en palabras: ", entrada); //captura el numero
	
		int numero = stoi(entrada[0]); //convierte la parte entera del numero a formato numerico
		
		string resultado = EscribirNumero(numero, 0, ""); //retorna el numero en letras
		if(numero==0) resultado = "Cero";
		
		if(entrada[1] != "") resultado += "con " + entrada[1] + " centavos"; // si se introdujo una parte decimal al numero al resultado, agregalo
		
		resultado[0] = toupper(resultado[0]); //Pone en mayusculas la primera palabra
		
		cout<< "El numero transformado a palabras es: " << endl << endl << resultado << endl << endl;
...
string EscribirNumero(int numero, int contador, string resultado){ //retorna el numero completo en palabras
	if(numero == 0){ //si el numero es 0
		return resultado; //retorna el resultado como esta
	}
	else{
		int auxiliar = numero;
		...
		if(caso != 0){ //si el grupo de digitos obtenidos no equivalen a 0
			...
			resultado = ConvertirPalabras(caso, (bool)contador, 100, "") + ObtenerTerminacion(contador, plural) + auxiliarPalabra;
		...
		return EscribirNumero(auxiliar/1000, contador + 1, resultado);
		//se corre el punto 3 posiciones a la izquierda
		//se aumenta el contador de la cantidad de grupos de digitos nombrados
	}
...
string ConvertirPalabras(int numero, bool terminacion, int divisor, string resultado){ 
	if(divisor == 0 || numero == 0){ //si el divisor o el numero llega a 0
		return resultado;
	}
	else{
		...
		if(divisor == 10 && clave > 0){ //el numero tiene 2 digitos y la decena es mayor que 0
			if(auxiliar % 10 == 0){ //si la unidad que acompaña a la decena es 0
				resultado += decenas[clave - 1] + " "; //10 diez, 20 veinte, 30 treinta, ...
				return resultado;	
			}
			else{
				if (clave == 1){ //si la decena es 1
					if(auxiliar % 10 > 5) resultado += "dieci"; //si la unidad que acompaña a la decena es mayor que 5, es decir, 16, 17, 18, ...
		...
		if(divisor == 1 && clave > 0){ //si tiene un solo digito y es diferente de 0
			...
			return resultado; //como es el ultimo caso finaliza el proceso	
		}
		
		return ConvertirPalabras(auxiliar % divisor, terminacion, divisor / 10, resultado);
		//nos quedamos con los dos digitos de la derecha
		//dividimos el divisor entre 10 para reducir el divisor
	}
}
